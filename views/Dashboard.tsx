...
const Dashboard: React.FC<DashboardProps> = ({ logs, settings }) => {
  ...
  const chartData = [
    { name: 'Protein', amount: totals.protein, goal: settings.dailyProteinGoal, color: '#34c759' },
    { name: 'Carbs', amount: totals.carbs, goal: settings.dailyCarbGoal, color: '#007aff' },
    { name: 'Fat', amount: totals.fat, goal: settings.dailyFatGoal, color: '#ff3b30' },
  ];
  ...
  return (
    <div className="flex-1 overflow-y-auto pb-24 bg-ios-bg">
      {/* Header */}
      <div className="pt-14 pb-8 px-6 bg-white rounded-b-[2rem] shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07)] mb-6">
      ...
      <div className="px-6 space-y-6">
        <div>
         ...
         <div className="mb-6">
           ...
           <div className="bg-white rounded-2xl p-5 shadow-[0_2px_10px_rgba(0,0,0,0.03)] border border-gray-50 h-64">
               <ResponsiveContainer width="100%" height="100%">
                   <BarChart data={chartData} layout="vertical" margin={{ top: 0, right: 30, left: 20, bottom: 0 }} barCategoryGap={20}>
                      <XAxis type="number" hide />
                      <YAxis dataKey="name" type="category" axisLine={false} tickLine={false} tick={{fontSize: 12, fill: '#6b7280', fontWeight: 500}} width={50} />
                      <Tooltip 
                        cursor={{fill: 'rgba(0,0,0,0.03)'}} 
                        contentStyle={{ borderRadius: '12px', border: 'none', boxShadow: '0 4px 15px rgba(0, 0, 0, 0.1)' }} 
                      />
                      <Bar dataKey="amount" radius={[0, 6, 6, 0]} barSize={24} background={{ fill: '#f3f4f6', radius: [0, 6, 6, 0] }}>
